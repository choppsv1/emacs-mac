...
- (void)dealloc
{
  ...
  MRC_RELEASE(self->notificationCategories);
#endif
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
  ...
  self->notificationCategories =
    [NSMutableSet<UNNotificationCategory *>
        setWithObjects:showCategory, genericCategory, dismissCategory, nil];

  MRC_RETAIN(self->notificationCategories);
...
_mac_notification_clicked(NSString *identifier,
                          ...
                          NSString *key)
{
  NSData *action_data = MRC_RETAIN([options valueForKey:@"on-action"]);
  NSData *close_data = MRC_RETAIN([options valueForKey:@"on-close"]);
  ...
  mac_within_lisp_deferred (^{
      ...
      Lisp_Object on_action = action_data ? mac_callable_from_data(action_data) : Qnil;
      Lisp_Object on_close = close_data ? mac_callable_from_data(close_data) : Qnil;
...
#endif /* ENABLE_NEW_NOTIFICATIONS */
                             ...
                             Lisp_Object arg_plist)
{
  ...
  if (STRINGP(sound_name))
    userNotification.soundName = [NSString stringWithUTF8LispString:sounds_name];
  ...
  switch (notif.activationType)
    ...
    case NSUserNotificationActivationTypeAdditionalActionClicked:
      mac_legacy_notification_clicked(userNotification, NO,
                                      [notif additionalActivationAction].identifier);
      break;
    }
}

#pragma clang diagnostic pop
#endif /* ENABLE_LEGACY_NOTIFICATIONS */
