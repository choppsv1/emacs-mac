...
- (void)dealloc
{
  ...
  MRC_RELEASE(notificationCategories);
  MRC_RELEASE(notificationIdentifiers);
#endif
...
mac_notification_clicked_lisp(void *arg)
{
  ...
  if ([response respondsToSelector:@selector(userText)] && [response userText].length)
      plist = Fplist_put(plist, intern(":user-text"), [[response userText] UTF8LispString]);
...
UNNotificationContent (e.g., the title can be found in :title if non-nil)
Additionally :identifier is set the the identifier for this
notification, :category is the category that defined the actions. All
values are strings except :badge which is a number.

usage: (mac-notification-send TITLE BODY &rest PARAMS)  */)
